...
from functional_recursion import recur, tail_recursive


def fib_standard_recursive(n, last_two=None):
    if last_two is None:
        last_two = (0, 1)
    if n == 0:
        return last_two[1]
    last_two = (last_two[1], sum(last_two))
    return fib_standard_recursive(n-1, last_two)


def fib_standard_loop(n):
    last_two = (0, 1)
    for _ in range(n):
        last_two = (last_two[1], sum(last_two))
    return last_two[1]
...
def print_last(result):
    number_of_digits = len(str(result))
